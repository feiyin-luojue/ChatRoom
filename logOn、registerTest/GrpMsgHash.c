...
/* 群聊哈希表消息取 */
int GpHashGet(GpHash* Gp, sqlite3* Data_Db, char* Group, char* sender, char* receiver, char* Message)
{
    if(Gp == NULL)
    {
        return NULL_PTR;
    ...
    int keyIdx = GpHashKeyId(Group, Gp->slotNums);
    int ret = GpMsgListGet(&(Gp->GpKeyId[keyIdx]), Data_Db, Group, sender, receiver, Message);
    return ret;
}
