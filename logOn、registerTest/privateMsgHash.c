...
static int MsgListGet(MsgList* List, sqlite3* Data_Db, char* Sender, char* Receiver, char* Message)
{
    if(List == NULL)
    {
        return NULL_PTR;
...
int hashMsgGet(MsgHash * msgHash, sqlite3* Data_Db, char* Sender, char* Receiver, char* Message)
{
    ...
    int ret = MsgListGet(&(msgHash->rcvKeyId[keyIdx]), Data_Db, Sender, Receiver, Message);
