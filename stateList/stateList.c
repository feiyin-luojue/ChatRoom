...
/* 修改用户状态 */
static int stateListModify(stateList * list, char* Account, int State);
...
/* 新增用户状态 */
int stateListInsert(stateList * list, char* Account, int Acceptfd)
{
    if(list == NULL)
    {
        return NULL_PTR;
    ...
    Node->state = ON_LINE;//登录时默认设置状态为仅在线
...
/* 修改指定账号状态为仅在线上 */
int stateOnlineModify(stateList * list, char* Account)
{
...
/* 修改指定账号状态为在群聊中 */
int stateGroupChatModify(stateList * list, char* Account)
{
